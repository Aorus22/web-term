export interface TerminalThemePreset {
  name: string       // CSS class name: wterm.theme-{name}
  label: string      // Display name
  colors: {
    foreground: string
    background: string
    cursor: string
    black: string        // color-0
    red: string          // color-1
    green: string        // color-2
    yellow: string       // color-3
    blue: string         // color-4
    magenta: string      // color-5
    cyan: string         // color-6
    white: string        // color-7
    brightBlack: string  // color-8
    brightRed: string    // color-9
    brightGreen: string  // color-10
    brightYellow: string // color-11
    brightBlue: string   // color-12
    brightMagenta: string// color-13
    brightCyan: string   // color-14
    brightWhite: string  // color-15
  }
}

/**
 * Map UI theme name to the closest matching terminal theme name.
 * Extracts the base name (removing -dark/-light suffix) and looks up
 * the corresponding terminal theme. Falls back to 'default' for dark
 * themes and 'light' for light themes if no match is found.
 */
export function getTerminalThemeForUITheme(uiThemeName: string): string {
	const knownTerminalThemes: Record<string, string> = {
		'default-dark': 'default',
		'default-light': 'light',
		'ocean-dark': 'ocean-dark',
		'ocean-light': 'ocean-light',
		'forest-dark': 'forest-dark',
		'forest-light': 'forest-light',
		'lavender-dark': 'lavender-dark',
		'lavender-light': 'lavender-light',
		'cherry-dark': 'cherry-dark',
		'cherry-light': 'cherry-light',
		'sunset-dark': 'sunset-dark',
		'sunset-light': 'sunset-light',
		'mono-dark': 'mono-dark',
		'mono-light': 'mono-light',
		'mint-dark': 'mint-dark',
		'mint-light': 'mint-light',
		'rose-dark': 'rose-dark',
		'rose-light': 'rose-light',
		'amber-dark': 'amber-dark',
		'amber-light': 'amber-light',
		'nord-dark': 'nord',
		'nord-light': 'nord',
		'midnight-dark': 'midnight-dark',
		'midnight-light': 'midnight-light',
		'dracula-dark': 'dracula',
		'monokai-dark': 'monokai',
		'catppuccin-dark': 'catppuccin',
		'gruvbox-dark': 'gruvbox-dark',
		'gruvbox-light': 'gruvbox-light',
		'tokyo-night-dark': 'tokyo-night',
		'everforest-dark': 'everforest-dark',
		'everforest-light': 'everforest-light',
		'rose-pine-dark': 'rose-pine-dark',
		'solarized-dark': 'solarized-dark',
		'solarized-light': 'solarized-light',
		'one-dark': 'one-dark',
		'one-light': 'one-dark',
		'github-dark': 'github-dark',
		'github-light': 'github-light',
		'ayu-dark': 'ayu-dark',
		'ayu-light': 'ayu-light',
		'kanagawa-dark': 'kanagawa-dark',
		'kanagawa-light': 'kanagawa-light',
		'zenburn-dark': 'zenburn-dark',
		'material-dark': 'material-dark',
		'material-light': 'material-light',
		'palenight-dark': 'palenight-dark',
		'darcula-dark': 'darcula-dark',
		'cobalt2-dark': 'cobalt2-dark',
		'night-owl-dark': 'night-owl-dark',
		'horizon-dark': 'horizon-dark',
		'synthwave-dark': 'synthwave-dark',
		'shades-of-purple-dark': 'shades-of-purple-dark',
		'oceanic-next-dark': 'oceanic-next-dark',
		'panda-dark': 'panda-dark',
		'cyberpunk-dark': 'cyberpunk-dark',
		'obsidian-dark': 'obsidian-dark',
		'arc-dark': 'arc-dark',
		'noctis-dark': 'noctis-dark',
		'blush-dark': 'blush-dark',
		'blush-light': 'blush-light',
		'sand-dark': 'sand-dark',
		'sand-light': 'sand-light',
		'sky-dark': 'sky-dark',
		'sky-light': 'sky-light',
		'plum-dark': 'plum-dark',
		'plum-light': 'plum-light',
		'crimson-dark': 'crimson-dark',
		'crimson-light': 'crimson-light',
		'lime-dark': 'lime-dark',
		'lime-light': 'lime-light',
		'orchid-dark': 'orchid-dark',
		'orchid-light': 'orchid-light',
		'teal-dark': 'teal-dark',
		'teal-light': 'teal-light',
		'charcoal-dark': 'charcoal-dark',
		'coffee-dark': 'coffee-dark',
		'coffee-light': 'coffee-light',
		'frost-dark': 'frost-dark',
		'frost-light': 'frost-light',
		'everglow-dark': 'everglow-dark',
	}

	if (knownTerminalThemes[uiThemeName]) return knownTerminalThemes[uiThemeName]

	return 'default'
}

export const terminalThemes: TerminalThemePreset[] = [
  {
    name: 'default',
    label: 'Default Dark',
    colors: {
      foreground: '#d4d4d4',
      background: '#1e1e1e',
      cursor: '#aeafad',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'monokai',
    label: 'Monokai',
    colors: {
      foreground: '#f8f8f2',
      background: '#272822',
      cursor: '#f8f8f0',
      black: '#272822',
      red: '#f92672',
      green: '#a6e22e',
      yellow: '#f4bf75',
      blue: '#66d9ef',
      magenta: '#ae81ff',
      cyan: '#a1efe4',
      white: '#f8f8f2',
      brightBlack: '#75715e',
      brightRed: '#f92672',
      brightGreen: '#a6e22e',
      brightYellow: '#f4bf75',
      brightBlue: '#66d9ef',
      brightMagenta: '#ae81ff',
      brightCyan: '#a1efe4',
      brightWhite: '#f9f8f5',
    }
  },
  {
    name: 'solarized-dark',
    label: 'Solarized Dark',
    colors: {
      foreground: '#839496',
      background: '#002b36',
      cursor: '#93a1a1',
      black: '#073642',
      red: '#dc322f',
      green: '#859900',
      yellow: '#b58900',
      blue: '#268bd2',
      magenta: '#d33682',
      cyan: '#2aa198',
      white: '#eee8d5',
      brightBlack: '#586e75',
      brightRed: '#cb4b16',
      brightGreen: '#586e75',
      brightYellow: '#657b83',
      brightBlue: '#839496',
      brightMagenta: '#6c71c4',
      brightCyan: '#93a1a1',
      brightWhite: '#fdf6e3',
    }
  },
  {
    name: 'solarized-light',
    label: 'Solarized Light',
    colors: {
      foreground: '#657b83',
      background: '#fdf6e3',
      cursor: '#586e75',
      black: '#eee8d5',
      red: '#dc322f',
      green: '#859900',
      yellow: '#b58900',
      blue: '#268bd2',
      magenta: '#d33682',
      cyan: '#2aa198',
      white: '#073642',
      brightBlack: '#93a1a1',
      brightRed: '#cb4b16',
      brightGreen: '#93a1a1',
      brightYellow: '#839496',
      brightBlue: '#657b83',
      brightMagenta: '#6c71c4',
      brightCyan: '#586e75',
      brightWhite: '#002b36',
    }
  },
  {
    name: 'light',
    label: 'Light',
    colors: {
      foreground: '#383a42',
      background: '#fafafa',
      cursor: '#526eff',
      black: '#383a42',
      red: '#e45649',
      green: '#50a14f',
      yellow: '#c18401',
      blue: '#4078f2',
      magenta: '#a626a4',
      cyan: '#0184bc',
      white: '#fafafa',
      brightBlack: '#a0a1a7',
      brightRed: '#e45649',
      brightGreen: '#50a14f',
      brightYellow: '#c18401',
      brightBlue: '#4078f2',
      brightMagenta: '#a626a4',
      brightCyan: '#0184bc',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'dracula',
    label: 'Dracula',
    colors: {
      foreground: '#f8f8f2',
      background: '#282a36',
      cursor: '#f8f8f2',
      black: '#21222c',
      red: '#ff5555',
      green: '#50fa7b',
      yellow: '#f1fa8c',
      blue: '#bd93f9',
      magenta: '#ff79c6',
      cyan: '#8be9fd',
      white: '#f8f8f2',
      brightBlack: '#6272a4',
      brightRed: '#ff6e6e',
      brightGreen: '#69ff94',
      brightYellow: '#ffffa5',
      brightBlue: '#d6acff',
      brightMagenta: '#ff92df',
      brightCyan: '#a4ffff',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'nord',
    label: 'Nord',
    colors: {
      foreground: '#d8dee9',
      background: '#2e3440',
      cursor: '#d8dee9',
      black: '#3b4252',
      red: '#bf616a',
      green: '#a3be8c',
      yellow: '#ebcb8b',
      blue: '#81a1c1',
      magenta: '#b48ead',
      cyan: '#88c0d0',
      white: '#e5e9f0',
      brightBlack: '#4c566a',
      brightRed: '#bf616a',
      brightGreen: '#a3be8c',
      brightYellow: '#ebcb8b',
      brightBlue: '#81a1c1',
      brightMagenta: '#b48ead',
      brightCyan: '#8fbcbb',
      brightWhite: '#eceff4',
    }
  },
  {
    name: 'one-dark',
    label: 'One Dark',
    colors: {
      foreground: '#abb2bf',
      background: '#282c34',
      cursor: '#528bff',
      black: '#282c34',
      red: '#e06c75',
      green: '#98c379',
      yellow: '#d19a66',
      blue: '#61afef',
      magenta: '#c678dd',
      cyan: '#56b6c2',
      white: '#abb2bf',
      brightBlack: '#5c6370',
      brightRed: '#e06c75',
      brightGreen: '#98c379',
      brightYellow: '#d19a66',
      brightBlue: '#61afef',
      brightMagenta: '#c678dd',
      brightCyan: '#56b6c2',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'github-dark',
    label: 'GitHub Dark',
    colors: {
      foreground: '#c9d1d9',
      background: '#0d1117',
      cursor: '#58a6ff',
      black: '#484f58',
      red: '#ff7b72',
      green: '#3fb950',
      yellow: '#d29922',
      blue: '#58a6ff',
      magenta: '#bc8cff',
      cyan: '#39c5cf',
      white: '#b1bac4',
      brightBlack: '#6e7681',
      brightRed: '#ffa198',
      brightGreen: '#56d364',
      brightYellow: '#e3b341',
      brightBlue: '#79c0ff',
      brightMagenta: '#d2a8ff',
      brightCyan: '#56d4dd',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'github-light',
    label: 'GitHub Light',
    colors: {
      foreground: '#24292f',
      background: '#ffffff',
      cursor: '#0969da',
      black: '#24292f',
      red: '#cf222e',
      green: '#1a7f37',
      yellow: '#9a6700',
      blue: '#0969da',
      magenta: '#8250df',
      cyan: '#1b7c83',
      white: '#6e7781',
      brightBlack: '#57606a',
      brightRed: '#a40e26',
      brightGreen: '#116329',
      brightYellow: '#4d2d00',
      brightBlue: '#0550ae',
      brightMagenta: '#6639ba',
      brightCyan: '#055d64',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'tokyo-night',
    label: 'Tokyo Night',
    colors: {
      foreground: '#a9b1d6',
      background: '#1a1b26',
      cursor: '#c0caf5',
      black: '#15161e',
      red: '#f7768e',
      green: '#9ece6a',
      yellow: '#e0af68',
      blue: '#7aa2f7',
      magenta: '#bb9af7',
      cyan: '#7dcfff',
      white: '#a9b1d6',
      brightBlack: '#414868',
      brightRed: '#f7768e',
      brightGreen: '#9ece6a',
      brightYellow: '#e0af68',
      brightBlue: '#7aa2f7',
      brightMagenta: '#bb9af7',
      brightCyan: '#7dcfff',
      brightWhite: '#c0caf5',
    }
  },
  {
    name: 'catppuccin',
    label: 'Catppuccin Mocha',
    colors: {
      foreground: '#cdd6f4',
      background: '#1e1e2e',
      cursor: '#f5e0dc',
      black: '#45475a',
      red: '#f38ba8',
      green: '#a6e3a1',
      yellow: '#f9e2af',
      blue: '#89b4fa',
      magenta: '#f5c2e7',
      cyan: '#94e2d5',
      white: '#bac2de',
      brightBlack: '#585b70',
      brightRed: '#f38ba8',
      brightGreen: '#a6e3a1',
      brightYellow: '#f9e2af',
      brightBlue: '#89b4fa',
      brightMagenta: '#f5c2e7',
      brightCyan: '#94e2d5',
      brightWhite: '#a6adc8',
    }
  },
  {
    name: 'gruvbox-dark',
    label: 'Gruvbox Dark',
    colors: {
      foreground: '#ebdbb2',
      background: '#282828',
      cursor: '#ebdbb2',
      black: '#282828',
      red: '#cc241d',
      green: '#98971a',
      yellow: '#d79921',
      blue: '#458588',
      magenta: '#b16286',
      cyan: '#689d6a',
      white: '#a89984',
      brightBlack: '#928374',
      brightRed: '#fb4934',
      brightGreen: '#b8bb26',
      brightYellow: '#fabd2f',
      brightBlue: '#83a598',
      brightMagenta: '#d3869b',
      brightCyan: '#8ec07c',
      brightWhite: '#ebdbb2',
    }
  },
  {
    name: 'ocean-dark',
    label: 'Ocean',
    colors: {
      foreground: '#FFFFFF',
      background: '#0C1929',
      cursor: '#2563EB',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'ocean-light',
    label: 'Ocean',
    colors: {
      foreground: '#0F172A',
      background: '#F0F9FF',
      cursor: '#2563EB',
      black: '#383a42',
      red: '#e45649',
      green: '#50a14f',
      yellow: '#c18401',
      blue: '#4078f2',
      magenta: '#a626a4',
      cyan: '#0184bc',
      white: '#fafafa',
      brightBlack: '#a0a1a7',
      brightRed: '#e45649',
      brightGreen: '#50a14f',
      brightYellow: '#c18401',
      brightBlue: '#4078f2',
      brightMagenta: '#a626a4',
      brightCyan: '#0184bc',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'forest-dark',
    label: 'Forest',
    colors: {
      foreground: '#FFFFFF',
      background: '#0C1F17',
      cursor: '#059669',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'forest-light',
    label: 'Forest',
    colors: {
      foreground: '#0F172A',
      background: '#F0FDF4',
      cursor: '#059669',
      black: '#383a42',
      red: '#e45649',
      green: '#50a14f',
      yellow: '#c18401',
      blue: '#4078f2',
      magenta: '#a626a4',
      cyan: '#0184bc',
      white: '#fafafa',
      brightBlack: '#a0a1a7',
      brightRed: '#e45649',
      brightGreen: '#50a14f',
      brightYellow: '#c18401',
      brightBlue: '#4078f2',
      brightMagenta: '#a626a4',
      brightCyan: '#0184bc',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'lavender-dark',
    label: 'Lavender',
    colors: {
      foreground: '#FFFFFF',
      background: '#131127',
      cursor: '#7C3AED',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'lavender-light',
    label: 'Lavender',
    colors: {
      foreground: '#0F172A',
      background: '#FAF5FF',
      cursor: '#7C3AED',
      black: '#383a42',
      red: '#e45649',
      green: '#50a14f',
      yellow: '#c18401',
      blue: '#4078f2',
      magenta: '#a626a4',
      cyan: '#0184bc',
      white: '#fafafa',
      brightBlack: '#a0a1a7',
      brightRed: '#e45649',
      brightGreen: '#50a14f',
      brightYellow: '#c18401',
      brightBlue: '#4078f2',
      brightMagenta: '#a626a4',
      brightCyan: '#0184bc',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'cherry-dark',
    label: 'Cherry',
    colors: {
      foreground: '#FFFFFF',
      background: '#1A0F14',
      cursor: '#E11D48',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'cherry-light',
    label: 'Cherry',
    colors: {
      foreground: '#0F172A',
      background: '#FFF5F5',
      cursor: '#E11D48',
      black: '#383a42',
      red: '#e45649',
      green: '#50a14f',
      yellow: '#c18401',
      blue: '#4078f2',
      magenta: '#a626a4',
      cyan: '#0184bc',
      white: '#fafafa',
      brightBlack: '#a0a1a7',
      brightRed: '#e45649',
      brightGreen: '#50a14f',
      brightYellow: '#c18401',
      brightBlue: '#4078f2',
      brightMagenta: '#a626a4',
      brightCyan: '#0184bc',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'sunset-dark',
    label: 'Sunset',
    colors: {
      foreground: '#FFFFFF',
      background: '#19140D',
      cursor: '#B45309',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'sunset-light',
    label: 'Sunset',
    colors: {
      foreground: '#0F172A',
      background: '#FFF7ED',
      cursor: '#D97706',
      black: '#383a42',
      red: '#e45649',
      green: '#50a14f',
      yellow: '#c18401',
      blue: '#4078f2',
      magenta: '#a626a4',
      cyan: '#0184bc',
      white: '#fafafa',
      brightBlack: '#a0a1a7',
      brightRed: '#e45649',
      brightGreen: '#50a14f',
      brightYellow: '#c18401',
      brightBlue: '#4078f2',
      brightMagenta: '#a626a4',
      brightCyan: '#0184bc',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'mono-dark',
    label: 'Monochrome',
    colors: {
      foreground: '#FAFAFA',
      background: '#09090B',
      cursor: '#A1A1AA',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'mono-light',
    label: 'Monochrome',
    colors: {
      foreground: '#09090B',
      background: '#FAFAFA',
      cursor: '#52525B',
      black: '#383a42',
      red: '#e45649',
      green: '#50a14f',
      yellow: '#c18401',
      blue: '#4078f2',
      magenta: '#a626a4',
      cyan: '#0184bc',
      white: '#fafafa',
      brightBlack: '#a0a1a7',
      brightRed: '#e45649',
      brightGreen: '#50a14f',
      brightYellow: '#c18401',
      brightBlue: '#4078f2',
      brightMagenta: '#a626a4',
      brightCyan: '#0184bc',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'mint-dark',
    label: 'Mint',
    colors: {
      foreground: '#FFFFFF',
      background: '#022C22',
      cursor: '#34D399',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'mint-light',
    label: 'Mint',
    colors: {
      foreground: '#0F172A',
      background: '#F0FDFA',
      cursor: '#0D9488',
      black: '#383a42',
      red: '#e45649',
      green: '#50a14f',
      yellow: '#c18401',
      blue: '#4078f2',
      magenta: '#a626a4',
      cyan: '#0184bc',
      white: '#fafafa',
      brightBlack: '#a0a1a7',
      brightRed: '#e45649',
      brightGreen: '#50a14f',
      brightYellow: '#c18401',
      brightBlue: '#4078f2',
      brightMagenta: '#a626a4',
      brightCyan: '#0184bc',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'rose-dark',
    label: 'Rose',
    colors: {
      foreground: '#FFFFFF',
      background: '#1C0D11',
      cursor: '#F43F5E',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'rose-light',
    label: 'Rose',
    colors: {
      foreground: '#0F172A',
      background: '#FFF1F2',
      cursor: '#E11D48',
      black: '#383a42',
      red: '#e45649',
      green: '#50a14f',
      yellow: '#c18401',
      blue: '#4078f2',
      magenta: '#a626a4',
      cyan: '#0184bc',
      white: '#fafafa',
      brightBlack: '#a0a1a7',
      brightRed: '#e45649',
      brightGreen: '#50a14f',
      brightYellow: '#c18401',
      brightBlue: '#4078f2',
      brightMagenta: '#a626a4',
      brightCyan: '#0184bc',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'amber-dark',
    label: 'Amber',
    colors: {
      foreground: '#FFFFFF',
      background: '#1C1408',
      cursor: '#F59E0B',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'amber-light',
    label: 'Amber',
    colors: {
      foreground: '#0F172A',
      background: '#FFFBEB',
      cursor: '#D97706',
      black: '#383a42',
      red: '#e45649',
      green: '#50a14f',
      yellow: '#c18401',
      blue: '#4078f2',
      magenta: '#a626a4',
      cyan: '#0184bc',
      white: '#fafafa',
      brightBlack: '#a0a1a7',
      brightRed: '#e45649',
      brightGreen: '#50a14f',
      brightYellow: '#c18401',
      brightBlue: '#4078f2',
      brightMagenta: '#a626a4',
      brightCyan: '#0184bc',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'midnight-dark',
    label: 'Midnight Indigo',
    colors: {
      foreground: '#FFFFFF',
      background: '#090B1E',
      cursor: '#6366F1',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'midnight-light',
    label: 'Midnight',
    colors: {
      foreground: '#0F172A',
      background: '#EEF2FF',
      cursor: '#4F46E5',
      black: '#383a42',
      red: '#e45649',
      green: '#50a14f',
      yellow: '#c18401',
      blue: '#4078f2',
      magenta: '#a626a4',
      cyan: '#0184bc',
      white: '#fafafa',
      brightBlack: '#a0a1a7',
      brightRed: '#e45649',
      brightGreen: '#50a14f',
      brightYellow: '#c18401',
      brightBlue: '#4078f2',
      brightMagenta: '#a626a4',
      brightCyan: '#0184bc',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'everforest-dark',
    label: 'Everforest',
    colors: {
      foreground: '#D3C6AA',
      background: '#2B3339',
      cursor: '#A7C080',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'everforest-light',
    label: 'Everforest',
    colors: {
      foreground: '#5C6A72',
      background: '#FDF6E3',
      cursor: '#8DA101',
      black: '#383a42',
      red: '#e45649',
      green: '#50a14f',
      yellow: '#c18401',
      blue: '#4078f2',
      magenta: '#a626a4',
      cyan: '#0184bc',
      white: '#fafafa',
      brightBlack: '#a0a1a7',
      brightRed: '#e45649',
      brightGreen: '#50a14f',
      brightYellow: '#c18401',
      brightBlue: '#4078f2',
      brightMagenta: '#a626a4',
      brightCyan: '#0184bc',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'rose-pine-dark',
    label: 'Rosé Pine',
    colors: {
      foreground: '#E0DEF4',
      background: '#191724',
      cursor: '#EBBCBA',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'ayu-dark',
    label: 'Ayu',
    colors: {
      foreground: '#B3B1AD',
      background: '#0A0E14',
      cursor: '#FFB454',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'ayu-light',
    label: 'Ayu',
    colors: {
      foreground: '#575F66',
      background: '#FAFAFA',
      cursor: '#FF6A00',
      black: '#383a42',
      red: '#e45649',
      green: '#50a14f',
      yellow: '#c18401',
      blue: '#4078f2',
      magenta: '#a626a4',
      cyan: '#0184bc',
      white: '#fafafa',
      brightBlack: '#a0a1a7',
      brightRed: '#e45649',
      brightGreen: '#50a14f',
      brightYellow: '#c18401',
      brightBlue: '#4078f2',
      brightMagenta: '#a626a4',
      brightCyan: '#0184bc',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'kanagawa-dark',
    label: 'Kanagawa',
    colors: {
      foreground: '#DCD7BA',
      background: '#1F1F28',
      cursor: '#7E9CD8',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'kanagawa-light',
    label: 'Kanagawa',
    colors: {
      foreground: '#69604D',
      background: '#F2ECBC',
      cursor: '#5E81AC',
      black: '#383a42',
      red: '#e45649',
      green: '#50a14f',
      yellow: '#c18401',
      blue: '#4078f2',
      magenta: '#a626a4',
      cyan: '#0184bc',
      white: '#fafafa',
      brightBlack: '#a0a1a7',
      brightRed: '#e45649',
      brightGreen: '#50a14f',
      brightYellow: '#c18401',
      brightBlue: '#4078f2',
      brightMagenta: '#a626a4',
      brightCyan: '#0184bc',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'zenburn-dark',
    label: 'Zenburn',
    colors: {
      foreground: '#DCDCCC',
      background: '#3F3F3F',
      cursor: '#8CD0D3',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'material-dark',
    label: 'Material',
    colors: {
      foreground: '#EEFFFF',
      background: '#263238',
      cursor: '#82AAFF',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'material-light',
    label: 'Material',
    colors: {
      foreground: '#546E7A',
      background: '#FAFAFA',
      cursor: '#2E7D32',
      black: '#383a42',
      red: '#e45649',
      green: '#50a14f',
      yellow: '#c18401',
      blue: '#4078f2',
      magenta: '#a626a4',
      cyan: '#0184bc',
      white: '#fafafa',
      brightBlack: '#a0a1a7',
      brightRed: '#e45649',
      brightGreen: '#50a14f',
      brightYellow: '#c18401',
      brightBlue: '#4078f2',
      brightMagenta: '#a626a4',
      brightCyan: '#0184bc',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'palenight-dark',
    label: 'Palenight',
    colors: {
      foreground: '#BFC7D5',
      background: '#292D3E',
      cursor: '#C792EA',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'darcula-dark',
    label: 'Darcula',
    colors: {
      foreground: '#A9B7C6',
      background: '#2B2B2B',
      cursor: '#CC7832',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'cobalt2-dark',
    label: 'Cobalt2',
    colors: {
      foreground: '#FFFFFF',
      background: '#193549',
      cursor: '#FFC600',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'night-owl-dark',
    label: 'Night Owl',
    colors: {
      foreground: '#D6DEEB',
      background: '#011627',
      cursor: '#82AAFF',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'horizon-dark',
    label: 'Horizon',
    colors: {
      foreground: '#DBD6D2',
      background: '#1C1E26',
      cursor: '#E95678',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'synthwave-dark',
    label: 'Synthwave \'84',
    colors: {
      foreground: '#E0E0E0',
      background: '#262335',
      cursor: '#F92AAD',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'shades-of-purple-dark',
    label: 'Shades of Purple',
    colors: {
      foreground: '#FFFFFF',
      background: '#2D2B55',
      cursor: '#FAD000',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'oceanic-next-dark',
    label: 'Oceanic Next',
    colors: {
      foreground: '#CDD3DE',
      background: '#1B2B34',
      cursor: '#EC5F67',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'panda-dark',
    label: 'Panda',
    colors: {
      foreground: '#E6E6E6',
      background: '#292A2B',
      cursor: '#19F9D8',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'cyberpunk-dark',
    label: 'Cyberpunk',
    colors: {
      foreground: '#FFFFFF',
      background: '#0D0A1A',
      cursor: '#FF007F',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'obsidian-dark',
    label: 'Obsidian',
    colors: {
      foreground: '#EBEBEB',
      background: '#0C0C0F',
      cursor: '#8B5CF6',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'arc-dark',
    label: 'Arc',
    colors: {
      foreground: '#D3DAE3',
      background: '#2F343F',
      cursor: '#5294E2',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'noctis-dark',
    label: 'Noctis',
    colors: {
      foreground: '#D3D8E0',
      background: '#1E2430',
      cursor: '#7CB7FF',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'blush-dark',
    label: 'Blush',
    colors: {
      foreground: '#FFFFFF',
      background: '#1E151F',
      cursor: '#D18EA6',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'blush-light',
    label: 'Blush',
    colors: {
      foreground: '#2C1A24',
      background: '#FFF5F7',
      cursor: '#C5657C',
      black: '#383a42',
      red: '#e45649',
      green: '#50a14f',
      yellow: '#c18401',
      blue: '#4078f2',
      magenta: '#a626a4',
      cyan: '#0184bc',
      white: '#fafafa',
      brightBlack: '#a0a1a7',
      brightRed: '#e45649',
      brightGreen: '#50a14f',
      brightYellow: '#c18401',
      brightBlue: '#4078f2',
      brightMagenta: '#a626a4',
      brightCyan: '#0184bc',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'sand-dark',
    label: 'Sand',
    colors: {
      foreground: '#FFFFFF',
      background: '#1D1A14',
      cursor: '#D4A96A',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'sand-light',
    label: 'Sand',
    colors: {
      foreground: '#3D3627',
      background: '#FDFBF7',
      cursor: '#B8914A',
      black: '#383a42',
      red: '#e45649',
      green: '#50a14f',
      yellow: '#c18401',
      blue: '#4078f2',
      magenta: '#a626a4',
      cyan: '#0184bc',
      white: '#fafafa',
      brightBlack: '#a0a1a7',
      brightRed: '#e45649',
      brightGreen: '#50a14f',
      brightYellow: '#c18401',
      brightBlue: '#4078f2',
      brightMagenta: '#a626a4',
      brightCyan: '#0184bc',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'sky-dark',
    label: 'Sky',
    colors: {
      foreground: '#FFFFFF',
      background: '#0A1628',
      cursor: '#38BDF8',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'sky-light',
    label: 'Sky',
    colors: {
      foreground: '#0F172A',
      background: '#F0F9FF',
      cursor: '#0284C7',
      black: '#383a42',
      red: '#e45649',
      green: '#50a14f',
      yellow: '#c18401',
      blue: '#4078f2',
      magenta: '#a626a4',
      cyan: '#0184bc',
      white: '#fafafa',
      brightBlack: '#a0a1a7',
      brightRed: '#e45649',
      brightGreen: '#50a14f',
      brightYellow: '#c18401',
      brightBlue: '#4078f2',
      brightMagenta: '#a626a4',
      brightCyan: '#0184bc',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'plum-dark',
    label: 'Plum',
    colors: {
      foreground: '#FFFFFF',
      background: '#1A1423',
      cursor: '#C084FC',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'plum-light',
    label: 'Plum',
    colors: {
      foreground: '#1A1423',
      background: '#FDF4FF',
      cursor: '#9333EA',
      black: '#383a42',
      red: '#e45649',
      green: '#50a14f',
      yellow: '#c18401',
      blue: '#4078f2',
      magenta: '#a626a4',
      cyan: '#0184bc',
      white: '#fafafa',
      brightBlack: '#a0a1a7',
      brightRed: '#e45649',
      brightGreen: '#50a14f',
      brightYellow: '#c18401',
      brightBlue: '#4078f2',
      brightMagenta: '#a626a4',
      brightCyan: '#0184bc',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'crimson-dark',
    label: 'Crimson',
    colors: {
      foreground: '#FFFFFF',
      background: '#1B0D0F',
      cursor: '#EF4444',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'crimson-light',
    label: 'Crimson',
    colors: {
      foreground: '#1B0D0F',
      background: '#FEF2F2',
      cursor: '#DC2626',
      black: '#383a42',
      red: '#e45649',
      green: '#50a14f',
      yellow: '#c18401',
      blue: '#4078f2',
      magenta: '#a626a4',
      cyan: '#0184bc',
      white: '#fafafa',
      brightBlack: '#a0a1a7',
      brightRed: '#e45649',
      brightGreen: '#50a14f',
      brightYellow: '#c18401',
      brightBlue: '#4078f2',
      brightMagenta: '#a626a4',
      brightCyan: '#0184bc',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'lime-dark',
    label: 'Lime',
    colors: {
      foreground: '#FFFFFF',
      background: '#0F1A0B',
      cursor: '#84CC16',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'lime-light',
    label: 'Lime',
    colors: {
      foreground: '#0F1A0B',
      background: '#F7FEE7',
      cursor: '#65A30D',
      black: '#383a42',
      red: '#e45649',
      green: '#50a14f',
      yellow: '#c18401',
      blue: '#4078f2',
      magenta: '#a626a4',
      cyan: '#0184bc',
      white: '#fafafa',
      brightBlack: '#a0a1a7',
      brightRed: '#e45649',
      brightGreen: '#50a14f',
      brightYellow: '#c18401',
      brightBlue: '#4078f2',
      brightMagenta: '#a626a4',
      brightCyan: '#0184bc',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'orchid-dark',
    label: 'Orchid',
    colors: {
      foreground: '#FFFFFF',
      background: '#15111B',
      cursor: '#D946EF',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'orchid-light',
    label: 'Orchid',
    colors: {
      foreground: '#15111B',
      background: '#FDF4FF',
      cursor: '#A21CAF',
      black: '#383a42',
      red: '#e45649',
      green: '#50a14f',
      yellow: '#c18401',
      blue: '#4078f2',
      magenta: '#a626a4',
      cyan: '#0184bc',
      white: '#fafafa',
      brightBlack: '#a0a1a7',
      brightRed: '#e45649',
      brightGreen: '#50a14f',
      brightYellow: '#c18401',
      brightBlue: '#4078f2',
      brightMagenta: '#a626a4',
      brightCyan: '#0184bc',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'teal-dark',
    label: 'Teal',
    colors: {
      foreground: '#FFFFFF',
      background: '#081A17',
      cursor: '#14B8A6',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'teal-light',
    label: 'Teal',
    colors: {
      foreground: '#081A17',
      background: '#F0FDFA',
      cursor: '#0D9488',
      black: '#383a42',
      red: '#e45649',
      green: '#50a14f',
      yellow: '#c18401',
      blue: '#4078f2',
      magenta: '#a626a4',
      cyan: '#0184bc',
      white: '#fafafa',
      brightBlack: '#a0a1a7',
      brightRed: '#e45649',
      brightGreen: '#50a14f',
      brightYellow: '#c18401',
      brightBlue: '#4078f2',
      brightMagenta: '#a626a4',
      brightCyan: '#0184bc',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'charcoal-dark',
    label: 'Charcoal',
    colors: {
      foreground: '#CCCCCC',
      background: '#121214',
      cursor: '#4B9CD3',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'coffee-dark',
    label: 'Coffee',
    colors: {
      foreground: '#EDE0D4',
      background: '#1E1710',
      cursor: '#C49A6C',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'coffee-light',
    label: 'Coffee',
    colors: {
      foreground: '#4A3728',
      background: '#FBF7F2',
      cursor: '#A67B5B',
      black: '#383a42',
      red: '#e45649',
      green: '#50a14f',
      yellow: '#c18401',
      blue: '#4078f2',
      magenta: '#a626a4',
      cyan: '#0184bc',
      white: '#fafafa',
      brightBlack: '#a0a1a7',
      brightRed: '#e45649',
      brightGreen: '#50a14f',
      brightYellow: '#c18401',
      brightBlue: '#4078f2',
      brightMagenta: '#a626a4',
      brightCyan: '#0184bc',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'frost-dark',
    label: 'Frost',
    colors: {
      foreground: '#FFFFFF',
      background: '#0F1923',
      cursor: '#7DD3FC',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'frost-light',
    label: 'Frost',
    colors: {
      foreground: '#0F1923',
      background: '#F0F9FF',
      cursor: '#0369A1',
      black: '#383a42',
      red: '#e45649',
      green: '#50a14f',
      yellow: '#c18401',
      blue: '#4078f2',
      magenta: '#a626a4',
      cyan: '#0184bc',
      white: '#fafafa',
      brightBlack: '#a0a1a7',
      brightRed: '#e45649',
      brightGreen: '#50a14f',
      brightYellow: '#c18401',
      brightBlue: '#4078f2',
      brightMagenta: '#a626a4',
      brightCyan: '#0184bc',
      brightWhite: '#ffffff',
    }
  },
  {
    name: 'everglow-dark',
    label: 'Everglow',
    colors: {
      foreground: '#E8E6E3',
      background: '#121212',
      cursor: '#BB86FC',
      black: '#1e1e1e',
      red: '#f44747',
      green: '#6a9955',
      yellow: '#d7ba7d',
      blue: '#569cd6',
      magenta: '#c586c0',
      cyan: '#4ec9b0',
      white: '#d4d4d4',
      brightBlack: '#808080',
      brightRed: '#f44747',
      brightGreen: '#6a9955',
      brightYellow: '#d7ba7d',
      brightBlue: '#569cd6',
      brightMagenta: '#c586c0',
      brightCyan: '#4ec9b0',
      brightWhite: '#ffffff',
    }
  }
]
