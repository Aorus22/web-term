...
/// Convert scroll delta to arrow keys when in alternate screen without mouse reporting.
...
mod tests {
    ...
    #[test]
    fn test_scroll_alt_screen_arrows() {
        let pt = AlacPoint::new(Line(0), Column(0));
        let mode = TermMode::ALT_SCREEN;

        let up = scroll_report(2, pt, 0, mode).unwrap();
